...
for f in files:
    ...
    if (file_name == "") or (
        ...
        try:
            ...
            sql = f"INSERT INTO all_files(file_name, current, total, processing, newsgroup_name) VALUE ('{filename}', 0, 0 ,1,'{group_name_fin}') ON DUPLICATE KEY UPDATE processing=1"
            # db_cursor = mysql_database.db_connection.cursor()
            db_cursor.execute(sql)
            ...
            if processing_message_counter > current_position_in_db:
                if processing_message_counter % 100 == 0:
                    ...
                    print(filename.replace(".mbox", "") + ": " + str(processing_message_counter) + " of " + str(
                        all_count) + " (" + str(percentage) + "%) | " + str(messages_per_minute1) + " msgs/min (" + str(
                        messages_per_minute1 * 60) + " hr, " + str(messages_per_minute1 * 60 * 24) + " day)")
                ...
                try:
                    ...
                    if number_of_rows1 == 0:
                        ...
                        try:
                            ...
                            print('Problem: ' + message['newsgroups'])
                    ...
                    sql = f"INSERT INTO all_files(file_name, current, total, processing, newsgroup_name) VALUE ('{filename}',{processing_message_counter},{all_count},1,'{group_name_fin}') ON DUPLICATE KEY UPDATE current={processing_message_counter}, total={all_count}, processing=1"
                    # db_cursor = mysql_database.db_connection.cursor()
                    db_cursor.execute(sql)
                    configuration.db_connection.commit()
                    sql_subject_id = db_cursor.lastrowid
                    ...
                    if processing_message_counter == all_count:
                        sql = f"INSERT INTO all_files(file_name, current, total, processing, newsgroup_name) VALUE ('{filename}',{processing_message_counter},{all_count},0,'{group_name_fin}') ON DUPLICATE KEY UPDATE current={processing_message_counter}, total={all_count}, processing=0"
                        # db_cursor = mysql_database.db_connection.cursor()
                        db_cursor.execute(sql)
                        configuration.db_connection.commit()
                        sql_subject_id = db_cursor.lastrowid
